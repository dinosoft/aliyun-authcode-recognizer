...
def get_auth_code_from_image(image):
    ...
    _, contours, hierarchy = cv2.findContours( image_th.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    if auth_code_img_rec is not None:
        for rec in auth_code_img_rec:
            ...
            resized = cv2.resize(cropped, (20, 30), interpolation=cv2.INTER_AREA)
